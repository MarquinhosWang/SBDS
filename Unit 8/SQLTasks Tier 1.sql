...
AND (b.memid <>0 AND b.slots * f.membercost > 30 )
...
LEFT JOIN Members as m
USING (memid)
WHERE b.starttime = '2012-09-14%') as subquery
...
SELECT name AS Facility, SUM(rv) AS revenue
FROM (
SELECT name, memid,
CASE WHEN memid != 0 THEN membercost * slots
	  ELSE guestcost * slots  END AS rv
FROM Bookings
